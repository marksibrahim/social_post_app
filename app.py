...
try:
    u_home = expanduser("~")
    sys.path.append(u_home + "/Desktop/social_post_app")
    app_path = u_home + "/Desktop/social_post_app/"
except Exception, e:
    print "uh oh! couldn't access app file on Desktop"
    print e
    sys.exit()
...
def open_txt(): 
    ...
    try:
        with open(app_path+ 'post.txt', 'r') as f:
            post_file = f.read()
        return post_file
    except Exception, e:
...
def write_new_post(post):
    ...
    try:
        with open(app_path + "post.txt", 'w') as f:
            f.write(content)
            f.close()
    except Exception, e:
...
def add_embed(url, tag, post):
    fb = url + "?mbid=social_facebook_post" + post + "_" + tag
    tw = url + "?mbid=social_twitter_post" + post + "_" + tag
    return fb, tw
...
def shorten(url):
    ...
    #ADD bitly access_token
    access_token = 1234
    bitly_url = "https://api-ssl.bitly.com/v3/shorten?access_token="+ access_token + "&longUrl=" + urllib.quote_plus(url) + "&format=txt" 
    try:
...
def return_full(fb, tw):            
    ...
    if connection:
        try:    
            ...
            print "couldn't open bitly's site"
...
using terms from application "Finder"
	...
	set voila to do shell script "python ~/Desktop/social_post_app/app.py " & mtag
	...
	#show result
	display dialog svoila default answer slinks with title "Here you go!"
